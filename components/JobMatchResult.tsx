...
export default function JobMatchResult({ data }: JobMatchResultProps) {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl space-y-5 sm:space-y-6">
      {/* Match Score */}
      <div className={`animate-scale-in rounded-2xl border bg-gradient-to-br p-6 text-center shadow-lg sm:rounded-3xl sm:p-10 ${getMatchBg(match_score)}`}>
        ...
        <div className="relative mx-auto h-32 w-32 sm:h-40 sm:w-40">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={`animate-count-up text-4xl font-bold sm:text-5xl ${getMatchColor(match_score)}`}>
              {match_score}
            </span>
        ...
        <div className="mt-4 sm:mt-5">
          <span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold sm:px-4 sm:py-1.5 ${getMatchLabelColor(match_score)}`}>
      ...
      {/* Missing Keywords */}
      <div className="animate-fade-in-up delay-200 rounded-2xl border border-white/60 bg-white/80 p-5 shadow-lg shadow-red-500/5 backdrop-blur-sm sm:rounded-3xl sm:p-8" style={{ animationFillMode: "both" }}>
        <div className="mb-4 flex items-center justify-center gap-2 sm:mb-5 sm:gap-3">
          ...
          <h3 className="text-base font-bold text-slate-900 sm:text-lg">Missing Keywords</h3>
        </div>
      ...
      {recommendations.length > 0 && (
        <div className="animate-fade-in-up delay-300 rounded-2xl border border-white/60 bg-white/80 p-5 shadow-lg shadow-violet-500/5 backdrop-blur-sm sm:rounded-3xl sm:p-8" style={{ animationFillMode: "both" }}>
          <div className="mb-4 flex items-center justify-center gap-2 sm:mb-5 sm:gap-3">
            ...
            <h3 className="text-base font-bold text-slate-900 sm:text-lg">Recommendations</h3>
          </div>
          <ul className="space-y-3 sm:space-y-4">
            {recommendations.map((rec, i) => (
              <li
                key={i}
                className="flex items-start gap-3 rounded-xl border border-slate-100 bg-gradient-to-r from-slate-50/80 to-violet-50/30 p-3 transition-all hover:border-violet-200 hover:shadow-md hover:shadow-violet-500/5 sm:gap-4 sm:rounded-2xl sm:p-5"
                style={{ animation: `slide-in-right 0.4s ease-out ${i * 0.1}s both` }}
              >
                <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-violet-500 to-fuchsia-500 text-[10px] font-bold text-white shadow-sm sm:h-7 sm:w-7 sm:text-xs">
                  {i + 1}
                </span>
                <span className="text-xs leading-relaxed text-slate-700 sm:text-sm">{rec}</span>
              </li>
            ))}
          </ul>
        </div>
