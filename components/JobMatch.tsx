...
export default function JobMatch({ resumeId, onResult }: JobMatchProps) {
  ...
  const handleMatch = async () => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/api/job-match/", {
        method: "POST",
        headers: {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl">
      <div className="rounded-3xl border border-white/60 bg-white/70 p-8 shadow-xl shadow-indigo-500/5 backdrop-blur-sm">
        {/* Header */}
        ...
        {/* Resume status */}
        <div className={`mb-5 flex items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm ${
          resumeId
        ...
        {/* Textarea */}
        <div className="mb-5">
          <label htmlFor="job-desc" className="mb-2 block text-sm font-semibold text-slate-700">
            Job Description
          </label>
          <textarea
            id="job-desc"
            ...
            className="w-full resize-none rounded-2xl border border-slate-200 bg-slate-50/50 px-4 py-3.5 text-sm text-slate-900 placeholder-slate-400 transition-all focus:border-indigo-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
          />
          <div className="mt-1.5 flex items-center justify-between text-xs text-slate-400">
            <span>Include requirements, skills, and qualifications</span>
        ...
        {/* Error */}
        {error && (
          <div className="animate-fade-in mb-5 flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">
